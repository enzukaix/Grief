...
getgenv().eteritum_debug = true 
if (getgenv().eteritum_loaded and not getgenv().eteritum_debug) then
    warn("Script Already Loaded!");
    return;
end;
getgenv().eteritum_loaded = true;
...
local function register_font(name, weight, style, asset)
    local path = "eteritum/rivals/fonts/" .. asset.id;
    local font_path = "eteritum/rivals/fonts/" .. name .. ".font";
...
task.spawn(function()
    for name, suffix in font_names do
        local path = "eteritum/rivals/fonts/" .. suffix;
        local font_data;

        if isfile(path) then
            font_data = readfile(path);
        else
            font_data = game:HttpGet("https://github.com/i77lhm/storage/raw/refs/heads/main/fonts/" .. suffix);
        end;

        local registered = register_font(name, 400, "Normal", {
            id = suffix,
            font = font_data,
        });

        fonts[name] = Font.new(registered, Enum.FontWeight.Regular, Enum.FontStyle.Normal);
    end;
end)
...
local Library = {
    ...
    FontColor = Color3.fromHex("8388aa");

    MainColor = Color3.fromHex("171717");
    BackgroundColor = Color3.fromHex("171717");

    AccentColor = Color3.fromHex("8388aa");
    DisabledAccentColor = Color3.fromRGB(110, 70, 90);

    OutlineColor = Color3.fromHex("2c2c2c");
    DisabledOutlineColor = Color3.fromRGB(32, 26, 36);

    DisabledTextColor = Color3.fromRGB(90, 70, 85);

    RiskColor = Color3.fromRGB(255, 80, 110);
...
ScreenGui:Destroy()
local BlurEffect = game:GetService("Lighting"):FindFirstChild("LinoriaBlur")
...
function KeybindsToggle:SetVisibility(bool)
    ...
    KeybindsToggleContainer.Size = UDim2.new(1, 0, 0, bool and 18 or 0)
    
    local transparency = bool and 0 or 1
    KeybindsToggleLabel.TextTransparency = transparency
    KeybindsToggleLabel.TextStrokeTransparency = transparency
    
    local boxTrans = KeybindsToggle.Normal and 1 or transparency
    if KeybindsToggleOuter then
        KeybindsToggleOuter.BackgroundTransparency = boxTrans
    end
    if KeybindsToggleInner then
        KeybindsToggleInner.BackgroundTransparency = boxTrans
    end
...
function KeyPicker:Update()
    ...
    if KeybindsToggle.Loaded then
        ...
        local enableCheck = if Info.EnableCheck then Info.EnableCheck() else true
        local ShowKeybind = enableCheck and KeyPicker.Value ~= "..." and KeyPicker.Value ~= "..."
        KeybindsToggle:SetVisibility(true)
        KeybindsToggle:SetText(string.format("[%s] %s (%s)", tostring(KeyPicker.DisplayValue), Info.Text, KeyPicker.Mode))
        local displayState = if KeyPicker.Mode == "Hold" then KeyPicker:GetState() else (if Info.EnableCheck then Info.EnableCheck() else State)
        KeybindsToggle:Display(displayState)
    end
    local XSize = 0
    ...
    if Layout and not Library.KeybindContainer:GetAttribute("SizeHooked") then
        ...
        Library:GiveSignal(Layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            ...
            for _, child in pairs(Library.KeybindContainer:GetChildren()) do
                if child:IsA("Frame") and child:GetAttribute("TargetVisible") == true then
                    anyVisible = true
                    break
                end
            end
    ...
    UpdateMenuOuterPos()
        ...
        PickOuter.InputBegan:Connect(function(PickerInput)
            if PickerInput.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                ActiveModifiers = if Input.KeyCode == Enum.KeyCode.Escape or Key == "..." then {} else ActiveModifiers
...
Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
    ...
    if KeyPicker.Value == "..." then return end
...
function BaseAddonsFuncs:AddColorPicker(Idx, Info)
        ...
        local PickerFrameOuter = Library:Create("Frame", {
            ...
            Size = UDim2.fromOffset(230, Info.Transparency and 297 or 279);
            Visible = false;
            ZIndex = 15;
            Parent = ScreenGui,
        })
        ...
        local CopyButtonsHolder = Library:Create("Frame", {
            BackgroundTransparency = 1;
            Position = UDim2.new(0, 4, 0, Info.Transparency and 272 or 254);
        ...
        Library:Create("UIListLayout", {
            ...
            Padding = UDim.new(0, 4);
            SortOrder = Enum.SortOrder.LayoutOrder;
            Parent = CopyButtonsHolder;
        })

        local CopyHexOuter = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
        ...
        local CopyHexInner = Library:Create("Frame", {
            BackgroundColor3 = Library.MainColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.new(1, 0, 1, 0);
        ...
        Library:AddToRegistry(CopyHexInner, {
            BackgroundColor3 = "MainColor";
            BorderColor3 = "OutlineColor";
        })
        ...
        Library:OnHighlight(CopyHexOuter, CopyHexOuter,
            { BorderColor3 = "AccentColor" },
            { BorderColor3 = "Black" }
        )

        CopyHexOuter.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                setclipboard("#" .. ColorPicker.Value:ToHex())
            end
        end)

        local CopyColorOuter = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Size = UDim2.new(0.5, -2, 1, 0);
            ZIndex = 17;
        ...
        local CopyColorInner = Library:Create("Frame", {
            BackgroundColor3 = Library.MainColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.new(1, 0, 1, 0);
            ZIndex = 18;
            Parent = CopyColorOuter;
        })

        Library:AddToRegistry(CopyColorInner, {
            BackgroundColor3 = "MainColor";
            BorderColor3 = "OutlineColor";
        })
        ...
        Library:OnHighlight(CopyColorOuter, CopyColorOuter,
            { BorderColor3 = "AccentColor" },
            { BorderColor3 = "Black" }
        )

        CopyColorOuter.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
...
Library:ApplyTextStroke(DropdownArrow)
Library:AddToRegistry(DropdownArrow, { TextColor3 = "FontColor" })
        ...
        function Dropdown:UpdateColors()
            ...
            DropdownArrow.TextColor3 = Dropdown.Disabled and Library.DisabledAccentColor or Library.FontColor
        end
        ...
        function Dropdown:OpenDropdown()
            ...
            TweenService:Create(DropdownArrow, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 180}):Play()
        ...
        function Dropdown:CloseDropdown()
            ...
            TweenService:Create(DropdownArrow, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 0}):Play()
...
function Library:CreateWindow(...)
    ...
    Library:AddToRegistry(WindowSubTitle, {
        TextColor3 = "AccentColor" 
    })
...
function Library:Toggle(Toggling)
    ...
    Library.BlurObject = game:GetService("Lighting"):FindFirstChild("LinoriaBlur")
    local BlurEffect = Library.BlurObject
    if not BlurEffect then
        Library.BlurObject = Instance.new("BlurEffect")
        Library.BlurObject.Name = "LinoriaBlur"
        Library.BlurObject.Size = 0
        Library.BlurObject.Parent = game:GetService("Lighting")
        BlurEffect = Library.BlurObject
    end
        BlurEffect.Size = Toggled and Library.BlurEnabled and 24 or 0
        if Toggled then
...
if getgenv().skip_getgenv_linoria ~= true then getgenv().Library = Library end end
